...
-- of the trips table. The predicate MATCHES finalizeArrivedGeo's WHERE exactly (incl. origin_port_id
-- IS NOT NULL) so null-origin arrived trips — which can never be finalized — never enter the index or
-- get re-scanned. Spec: assistant/ANALYTICS_SCHEMA.md. Idempotent; safe to re-run.
CREATE INDEX IF NOT EXISTS ix_trips_arrived_unfinalized ON trips (id)
  WHERE status = 'arrived' AND distance_km IS NULL AND origin_port_id IS NOT NULL;
